...
class LSTMD(Gens):
    ...
    def init_real_trainng(self):
        ...
        with tf.variable_scope("generator_lstm", reuse=None, initializer=initializer):
            generator = Generator(num_vocabulary=self.vocab_size, batch_size=self.batch_size,hidden_dim=self.hidden_dim, sequence_length=self.sequence_length,
                                generator_name='generator_lstm', learning_rate=self.gen_lr, grad_clip=self.grad_clip,
        ...
        # discriminator　dataloder; Need to load equal amount of positive and negative samples
    ...
    def train_real(self):
        ...
        saver_variables = slim.get_variables_to_restore(include=["generator_lstm"])
        ...
        if self.restore:
            ...
            self.d1_fake_codes = self.get_fake_code(os.path.join(self.output_path, f"d1_fake_keep_c{filname}_train.pkl"))
            self.dis_train_data_loader.load_train_data_list(self.train_code, self.d1_fake_codes)
            self.dis_valid_data_loader.load_train_data_list_file(self.valid_code, os.path.join(self.output_path, f"d1_fake_keep_c{filname}_valid.txt"))
            acc_valid_best = 0
            for epoch in range(self.train_dis_num):
                print("d1 epoch:" + str(epoch))
                self.train_discriminator(self.discriminator_d1)

                accuracy_valid, loss_valid = self.get_distance(os.path.join(self.output_path, f"d1_fake_keep_c{filname}_valid.txt"), self.discriminator_d1, \
    ...
    def lm_vs_rlm(self, model_path, fake_train_code_file, fake_test_code_file):
        ...
        with tf.variable_scope("generator_lstm", reuse=None, initializer=initializer):
            generator = Generator(num_vocabulary=self.vocab_size, batch_size=self.batch_size, hidden_dim=self.hidden_dim,
        ...
        if "tmp" in fake_train_code_file:
            ...
            fake_train_code = self.get_fake_code(fake_train_code_file)#The filtered sample is stored sentence by sentence, so it needs to be read cyclically
        ...
        with tf.variable_scope("re_generator", reuse=None, initializer=initializer):
            generator = Generator(num_vocabulary=self.vocab_size, batch_size=self.batch_size,
                                  ...
                                  sequence_length=self.sequence_length,
                                  generator_name='re_generator', learning_rate=self.gen_lr, grad_clip=self.grad_clip,
                                  dropout_keep_prob=self.g_dropout_keep_prob, start_token=self.start_token, \
                                  num_layers_gen=self.num_layers_gen)
